...
def load_dataset(seq_type, emb_type, cell_line, max_len, seed):
    ...
    gene_symbols = pickle.load(open('../data/gene_ann_data.pkl','rb'))['gene_symbol']
    test_gene = pickle.load(open('./data/HCT116_ess_gene_without_ppi_ids.pkl','rb'))['test_gene']
...
#     train_indexes = list(set(ess_indexes + non_indexes) - set(test_indexes))
    ...
    # 将测试集设定为没有相互作用信息的蛋白质
    test_indexes = [gene_symbols.index(gene) for gene in test_gene]
    train_indexes = list(set(ess_indexes + non_indexes) - set(test_indexes))

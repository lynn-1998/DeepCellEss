...
def best_pre_thr(y_true, y_score):
    """ Calculate the best threshold  with pre """
    best_thr = 0.5
    best_pre = 0
    for thr in range(1,100):
        thr /= 100
        ...
        if pre>best_pre:
            best_pre = pre
            best_thr = thr 
    return best_thr, best_pre


def best_mcc_thr(y_true, y_score):
    """ Calculate the best threshold  with mcc """
    best_thr = 0.5
    best_mcc = 0
    for thr in range(1,100):
        thr /= 100
...
def best_f1_mcc_thr(y_true, y_score):
    """ 
    Calculate the best threshold point on the MCC-F1 curve closest to (1,1)
    Reference: https://arxiv.org/ftp/arxiv/papers/2006/2006.11278.pdf
    """
    best_thr = 0.5
    shortest_dis = 1.0
    for thr in range(1,100):
